...
- Fixed the Python CI workflow to install `pytest` before running `python -m pytest -v`.
- Fixed the orchestrator publish image build to use Go 1.24, matching `go.mod` and the CI toolchain.
